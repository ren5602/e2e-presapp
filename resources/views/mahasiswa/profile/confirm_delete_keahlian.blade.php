...
@else
    <div class="modal-header">
        <h5 class="modal-title">Hapus bidang_keahlianOrganisasi</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
        </button>
    </div>
    <div class="modal-body">
        ...
        <table class="table table-sm table-bordered table-striped">
            <tr>
                ...
                <td class="col-9">{{ $keahlian->bidang_keahlian->bidang_keahlian_nama }}</td>
            </tr>
        </table>
        <form action="{{ url('/profile/mahasiswa/keahlian/' . $keahlian->keahlian_mahasiswa_id) }}" method="POST" id="form-delete-keahlian">
            @csrf
            @method('DELETE')
